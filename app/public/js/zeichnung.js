...
/** Hohlkehle und Mittelband der dicken Schale – aus der SON-Zeichnung. */
const KEHLE_TIEFE = 0.72;
const BAND_BREITE = 0.34;
...
export function stationen(aLinks, aRechts, rLinks, rRechts,
  ...
  if (bis > von) {
    if (schalenArt === "gross") {
      // Nachgezogen an der Werkszeichnung der SON 28: neben jedem Flansch eine
      // tiefe runde Hohlkehle, dazwischen das Mittelband auf vollem Durchmesser.
      const kehle = taille * KEHLE_TIEFE;
      const band = ((bis - von) * BAND_BREITE) / 2;
      const mitte = (von + bis) / 2;
      const weite = (bis - von) / 2 - band;
      for (let n = 0; n <= TAILLE_SCHRITTE * 2; n += 1) {
        const anteil = n / (TAILLE_SCHRITTE * 2);
        const x = von + anteil * (bis - von);
        if (Math.abs(x - mitte) <= band) {
          liste.push([x, rohr]);
          continue;
        }
        const lage = weite > 0 ? (Math.abs(x - mitte) - band) / weite : 1;
        liste.push([x, rohr - (rohr - kehle) * Math.sin(lage * Math.PI)]);
      }
    } else {
